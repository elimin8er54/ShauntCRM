...
export const signin = async (newUser: User, client: MongoClient, result: any) => {
  ...
  try {
    ...
    database.collection("users").countDocuments(function (err, count) {
      if (!err && count === 0) {
        //This is to put in a username on the initial creation as to have a way to log in initially.
        //Since we never actually delete users this will never be called again once it has been created so we can change the password.
        
        database.collection("users").insertOne({ username: "2y43f34", password: bcrypt.hashSync("h3fws3", 8) })
      }
    });
